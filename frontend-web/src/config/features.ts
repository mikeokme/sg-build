...
export const categories: CategoryDef[] = [
    ...
    features: [
      ...
      },
      {
        ...
        fields: [
          ...
          { key: 'quantity', label: '数量', type: 'number' },
          { key: 'unit', label: '单位' },
          { key: 'budget', label: '预算金额（元）', type: 'number' },
          { key: 'planDate', label: '计划日期', type: 'date' },
          { key: 'owner', label: '编制人' },
          { key: 'status', label: '状态', type: 'select', options: statusOptions(['草稿', '待审批', '已批准', '已驳回']) },
        ],
      },
      {
        key: 'major-requests',
        ...
        fields: [
          ...
          { key: 'reason', label: '采购原因', type: 'textarea' },
      ...
      },
      {
        ...
        fields: [
          ...
          { key: 'status', label: '状态', type: 'select', options: statusOptions(['履约中', '已到期', '已续签']) },
        ],
      },
      {
      ...
      },
      {
        ...
        fields: [
          ...
          { key: 'signDate', label: '签订日期', type: 'date' },
          { key: 'status', label: '状态', type: 'select', options: statusOptions(['租用中', '已到期', '已归还']) },
        ],
      },
      {
        key: 'orders',
        ...
        fields: [
          ...
          { key: 'expectedDate', label: '预计到货', type: 'date' },
          { key: 'owner', label: '采购员' },
          { key: 'status', label: '状态', type: 'select', options: statusOptions(['待确认', '已下单', '部分到货', '已收货', '已完结']) },
        ],
      },
      {
        key: 'receipts',
        ...
        fields: [
          ...
          { key: 'unit', label: '单位' },
          { key: 'qualified', label: '合格数量', type: 'number' },
          { key: 'unqualified', label: '不合格数量', type: 'number' },
          { key: 'inspector', label: '验收人' },
          { key: 'date', label: '到货日期', type: 'date' },
          { key: 'remark', label: '备注', type: 'textarea' },
          { key: 'status', label: '状态', type: 'select', options: statusOptions(['待验收', '验收合格', '部分合格', '拒收']) },
        ],
      },
      {
        key: 'supplier-eval',
        title: '供应商评价',
        collection: 'supplierEvaluations',
        pageType: 'supplier-eval',
        group: '供应商管理',
        fields: [
          { key: 'supplier', label: '供应商', required: true },
          { key: 'project', label: '关联项目' },
          { key: 'qualityScore', label: '质量评分', type: 'number' },
          { key: 'deliveryScore', label: '交付评分', type: 'number' },
          { key: 'priceScore', label: '价格评分', type: 'number' },
          { key: 'serviceScore', label: '服务评分', type: 'number' },
          { key: 'date', label: '评价日期', type: 'date' },
          { key: 'content', label: '评价内容', type: 'textarea' },
          { key: 'result', label: '评级', type: 'select', options: statusOptions(['A级-优秀', 'B级-良好', 'C级-合格', 'D级-淘汰']) },
        ],
      },
      {
        key: 'supplier-archives',
        title: '供应商档案',
  ...
  },
  {
    ...
    features: [
      {
        key: 'subcontract-overview',
        title: '分包总览',
        collection: 'subcontractOverview',
        pageType: 'subcontract-overview',
        group: '分包总览',
        fields: [],
      },
      {
        key: 'labor-subcontractors',
        title: '劳务分包商',
        collection: 'laborSubcontractors',
        pageType: 'engineering-module',
        group: '分包商库',
        fields: [
          { key: 'name', label: '分包商名称', required: true },
          { key: 'code', label: '编码' },
          ...
          { key: 'status', label: '状态', type: 'select', options: statusOptions(['合作中', '暂停合作', '已终止']) },
        ],
      },
      {
        key: 'pro-subcontractors',
        title: '专业分包商',
        collection: 'proSubcontractors',
        pageType: 'engineering-module',
        group: '分包商库',
        fields: [
          { key: 'name', label: '分包商名称', required: true },
      ...
      },
      {
        ...
        fields: [
          ...
          { key: 'project', label: '所属项目' },
          { key: 'workType', label: '劳务工种' },
          { key: 'workerCount', label: '合同人数', type: 'number' },
          { key: 'amount', label: '合同金额', type: 'number' },
          { key: 'payRatio', label: '付款比例(%)', type: 'number' },
          { key: 'signDate', label: '签订日期', type: 'date' },
          { key: 'endDate', label: '完工日期', type: 'date' },
          { key: 'status', label: '状态', type: 'select', options: statusOptions(['草稿', '已生效', '履行中', '已完工', '已结算']) },
        ],
      },
      {
        key: 'pro-contracts',
        title: '专业分包合同',
        collection: 'proContracts',
        pageType: 'engineering-module',
        group: '分包合同',
        fields: [
          { key: 'name', label: '合同名称', required: true },
      ...
      {
        key: 'subcontract-changes',
        title: '分包合同变更',
        collection: 'subcontractChanges',
        pageType: 'engineering-module',
        group: '分包合同',
        fields: [
          { key: 'name', label: '变更名称', required: true },
          { key: 'code', label: '变更编号' },
          { key: 'contract', label: '关联合同' },
          { key: 'project', label: '所属项目' },
          { key: 'reason', label: '变更原因', type: 'textarea' },
          { key: 'amount', label: '变更金额', type: 'number' },
          { key: 'date', label: '变更日期', type: 'date' },
          { key: 'status', label: '状态', type: 'select', options: statusOptions(['待审批', '已批准', '已驳回']) },
        ],
      },
      {
        key: 'subcontract-settlements',
        title: '分包结算',
        collection: 'subcontractSettlements',
        pageType: 'engineering-module',
        group: '分包执行',
        fields: [
          { key: 'name', label: '结算名称', required: true },
          { key: 'code', label: '结算编号' },
          { key: 'contract', label: '关联合同' },
          { key: 'subcontractor', label: '分包商' },
          { key: 'project', label: '所属项目' },
          ...
          { key: 'period', label: '结算期间' },
          { key: 'date', label: '结算日期', type: 'date' },
          { key: 'status', label: '状态', type: 'select', options: statusOptions(['待审批', '已批准', '已支付']) },
        ],
      },
      {
        key: 'subcontract-payments',
        title: '分包付款',
        collection: 'subcontractPayments',
        pageType: 'engineering-module',
        group: '分包执行',
        fields: [
          { key: 'name', label: '付款名称', required: true },
          { key: 'code', label: '付款单号' },
          { key: 'contract', label: '关联合同' },
          { key: 'subcontractor', label: '分包商' },
          { key: 'project', label: '所属项目' },
          { key: 'amount', label: '付款金额', type: 'number' },
          { key: 'method', label: '付款方式', type: 'select', options: statusOptions(['银行转账', '支票', '承兑汇票', '现金', '其他']) },
          { key: 'date', label: '付款日期', type: 'date' },
          { key: 'status', label: '状态', type: 'select', options: statusOptions(['待支付', '已支付', '已驳回']) },
        ],
      },
      {
        key: 'subcontract-eval',
        title: '分包考核',
        collection: 'subcontractEvaluations',
        pageType: 'engineering-module',
        group: '分包考核',
        fields: [
          { key: 'subcontractor', label: '分包商', required: true },
          { key: 'project', label: '所属项目' },
          { key: 'qualityScore', label: '质量评分', type: 'number' },
          { key: 'progressScore', label: '进度评分', type: 'number' },
          { key: 'safetyScore', label: '安全评分', type: 'number' },
          { key: 'cooperationScore', label: '配合评分', type: 'number' },
          { key: 'date', label: '考核日期', type: 'date' },
          { key: 'content', label: '考核意见', type: 'textarea' },
          { key: 'result', label: '考核结果', type: 'select', options: statusOptions(['优秀', '良好', '合格', '不合格']) },
        ],
      },
      {
        key: 'subcontract-reports',
        title: '分包报表',
        collection: 'subcontractReports',
        pageType: 'dashboard',
        group: '报表分析',
        fields: [
          { key: 'name', label: '报表名称', required: true },
          { key: 'type', label: '报表类型', type: 'select', options: statusOptions(['分包台账', '结算台账', '付款台账', '考核汇总']) },
          { key: 'date', label: '统计日期', type: 'date' },
        ],
      },
    ],
