...
export function ConstructionLogsPage({ feature, categoryTitle, categoryKey }: { feature: FeatureDef; categoryTitle: string; categoryKey: string }) {
  ...
  const handleDelete = async (item: any) => {
    if (!confirm(`确认删除 ${item.date || ''} 的日志吗？`)) return;
    const token = localStorage.getItem('token');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {allowCreate ? (
          <Button onClick={openCreate} className="bg-blue-600 hover:bg-blue-700"><Plus className="w-4 h-4 mr-2" />填写日志</Button>
        ) : (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: '日志总数', value: items.length, tone: 'text-blue-600 bg-blue-50' },
          { label: '累计出勤（人次）', value: totalLabor.toLocaleString(), tone: 'text-emerald-600 bg-emerald-50' },
          { label: '涉及项目', value: projects.length, tone: 'text-purple-600 bg-purple-50' },
          { label: '记录问题', value: hasIssues, tone: 'text-amber-600 bg-amber-50' },
        ].map((s) => (
          <Card key={s.label}>
            <CardContent className="p-4">
              <p className="text-xl font-bold text-gray-900">{s.value}</p>
              <p className={`text-xs mt-1 inline-flex items-center gap-1 px-2 py-0.5 rounded-full ${s.tone}`}>{s.label}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center gap-3 pb-3 flex-wrap">
          <CardTitle className="text-base font-semibold">施工日志流水</CardTitle>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-16 text-gray-400">
              <p>暂无日志</p>
              {allowCreate && <p className="text-sm mt-1">点击右上角「填写日志」记录今日施工情况</p>}
            </div>
          ) : (
                  ...
                  <div key={it.id} className="border border-gray-100 rounded-xl p-4 hover:border-blue-200 hover:shadow-sm transition-all">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center flex-shrink-0">
                          <span className="text-[10px] font-bold text-blue-600 leading-tight text-center">
                            {(it.date || '').slice(8, 10)}<br />{(it.date || '').slice(5, 7)}月
                          </span>
                        </span>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <Badge variant="outline" className="gap-1 border-0 text-xs bg-gray-100 text-gray-600"><Users className="w-3 h-3" />{it.labor || 0}人</Badge>
                    ...
                    <div className="mt-3 space-y-2">
                      ...
                      {it.issues && it.issues !== '无' ? (
                        ...
                        <p className="text-xs text-emerald-600 flex items-center gap-1.5">
                          <AlertCircle className="w-3 h-3" />当日无问题
                        </p>
                      )}
                    </div>
                    {it.recorder && (
                      <p className="mt-2 text-[10px] text-gray-400">记录人：{it.recorder}</p>
                    )}
                  </div>
                );
              })}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editing ? '编辑' : '填写'}施工日志</DialogTitle></DialogHeader>
