  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Badge variant="secondary" className="text-xs">{items.length} 条数据</Badge>
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card><CardContent className="p-4"><p className="text-2xl font-semibold text-blue-600">{items.length}</p><p className="text-xs text-gray-500 mt-1">总记录数</p></CardContent></Card>
            {numFields.length > 0 && (
              <Card><CardContent className="p-4"><p className="text-2xl font-semibold text-emerald-600">{totalAmount.toLocaleString()}</p><p className="text-xs text-gray-500 mt-1">{numFields[0].label}合计</p></CardContent></Card>
            )}
            <Card><CardContent className="p-4"><p className="text-2xl font-semibold text-orange-600">{selectStats.reduce((s, [, n]) => s + n, 0)}</p><p className="text-xs text-gray-500 mt-1">已分类记录</p></CardContent></Card>
            <Card><CardContent className="p-4"><p className="text-2xl font-semibold text-purple-600">{textFields.length > 0 ? new Set(items.map((it) => it[textFields[0].key])).size : items.length}</p><p className="text-xs text-gray-500 mt-1">{firstTextKey || '分类'} 数量</p></CardContent></Card>
          </div>

          {selectStats.length > 0 && (
            <Card>
              <CardHeader><CardTitle className="text-base font-semibold">按「{selectFields[0].label}」分布</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                {selectStats.map(([label, count], i) => {
                  const pct = items.length ? Math.round((count / items.length) * 100) : 0;
                  return (
                    <div key={label}>
                      <div className="flex justify-between text-sm mb-1"><span>{label}</span><span className="text-gray-500">{count} 条 · {pct}%</span></div>
          ...
          <Card>
            <CardHeader><CardTitle className="text-base font-semibold">明细数据</CardTitle></CardHeader>
            <CardContent>
              {items.length === 0 ? (
                <div className="text-center py-12 text-gray-400"><p>暂无数据</p></div>
              ) : (
                    ...
                    <div key={it.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400 truncate">
                          {textFields.slice(1).map((f) => `${f.label}: ${it[f.key] ?? '-'}`).join(' · ')}
                        </p>
                      </div>
